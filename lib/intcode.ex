defmodule Intcode do
  ...
  """
  def restore(program, value_1, value_2) do
    program
    |> List.replace_at(1, value_1)
    |> List.replace_at(2, value_2)
    |> run()
  end
  ...
  """
  def run(program, pos \\ 0) do
    case Enum.slice(program, pos, @step) do
      [@halt | _] ->
        program

      [operation, left_input_pos, right_input_pos, output_pos]
      when operation in [@add, @multiply] ->
        left = Enum.at(program, left_input_pos)
        right = Enum.at(program, right_input_pos)

        result = calculate(operation, left, right)

        program
        |> List.replace_at(output_pos, result)
        |> run(pos + @step)
    end
  end
