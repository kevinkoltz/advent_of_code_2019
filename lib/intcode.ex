defmodule Intcode do
  ...
  """
  def restore(program, noun, verb) do
    program
  ...
  """
  @range 0..99
  def complete_gravity_assist(program, required_output \\ 19_690_720) do
    combinations = for noun <- @range, verb <- @range, do: {noun, verb}

    {noun, verb} =
      combinations
      |> Enum.find(fn {noun, verb} ->
        case restore(program, noun, verb) do
          [^required_output | _] -> true
          _ -> false
        end
      end)
  ...
  2,3,0,3,99 becomes 2,3,0,6,99 (3 * 2 = 6).
      ...
      iex> Intcode.run([2,3,0,3,99])
      [2,3,0,6,99]

  2,4,4,5,99,0 becomes 2,4,4,5,99,9801 (99 * 99 = 9801).

      iex> Intcode.run([2,4,4,5,99,0])
      [2,4,4,5,99,9801]
  ...
  def run(program, pointer \\ 0) do
    case Enum.slice(program, pointer, @step) do
      [@halt | _] ->
        program

      [opcode, param_1, param_2, param_3] when opcode in [@add, @multiply] ->
        left = Enum.at(program, param_1)
        right = Enum.at(program, param_2)

        result = calculate(opcode, left, right)

        program
        |> List.replace_at(param_3, result)
        |> run(pointer + @step)
    end
  end

  defp calculate(@add, left, right), do: left + right
  defp calculate(@multiply, left, right), do: left * right
end
